(function (global, factory) {
  ...
  var Store = /*#__PURE__*/function () {
    function Store(options) {
      ...
      this._actions = {};
      this._mutations = {};
      this._getters = {}; // 收集通过 store.subcribe 订阅状态变更事件的处理函数 fn
      // 当 mutation 执行时，触发全部订阅事件执行，返回当前 mutation 和更新后的状态

      this._subscribes = []; // 1,模块收集：options 格式化 -> Vuex 模块树
      ...
      resetStoreVM(this, state); // 依次执行 options 选项中的 plugins 插件,传入当前 store 实例
  ...
  var installModule = function installModule(store, rootState, path, module) {
    ...
    module.forEachMutation(function (mutation, key) {
      ...
      store._mutations[namespace + key].push(function (payload) {
        ...
        mutation.call(store, getState(store, path), payload); // 当 mutation 执行时，依次执行 store.subscribe 状态变更事件订阅的处理函数 fn

        store._subscribes.forEach(function (fn) {
          console.log('状态更新，依次执行订阅处理'); // fn(mutation, rootState)
